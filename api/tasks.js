...
async function getTaskFile(owner, repo, id) {
  ...
  try {
    ...
    return { task: JSON.parse(Buffer.from(file.content || '', 'base64').toString('utf8')), sha: file.sha };
  } catch (error) {
...
async function saveTask(owner, repo, task, sha, message) {
  ...
  return github(`/repos/${owner}/${repo}/contents/${path}`, { method: 'PUT', body: JSON.stringify({ message, content, sha }) });
}

async function listTasks(owner, repo) {
  let entries;
  try { entries = await github(`/repos/${owner}/${repo}/contents/.zvelclaw/tasks`); }
  catch (error) { if (/not found/i.test(error.message)) return []; throw error; }
  ...
  const tasks = await Promise.all(files.map(async (entry) => {
    try { return JSON.parse(Buffer.from((await github(`/repos/${owner}/${repo}/contents/${entry.path}`)).content || '', 'base64').toString('utf8')); }
    catch { return null; }
  }));
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'execute') {
      ...
      task.state = 'executing'; task.updatedAt = now; task.events = Array.isArray(task.events) ? task.events : [];
      ...
      task.gate = gate; task.state = gate.passed ? 'gated' : 'failed'; task.updatedAt = new Date().toISOString();
    ...
    if (action === 'create-pr') {
      ...
      task.github = githubResult; task.state = 'pr_created'; task.updatedAt = new Date().toISOString();
    ...
    const content = Buffer.from(`${JSON.stringify(task, null, 2)}\n`).toString('base64');
    const body = await github(`/repos/${owner}/${repo}/contents/${path}`, { method: 'PUT', body: JSON.stringify({ message: `task: create ${id}`, content }) });
    send(res, 201, { task, manifest: path, commit: body?.commit?.sha || null, url: body?.content?.html_url || null });
  } catch (error) { send(res, 500, { error: error.message || 'Task request failed.' }); }
}
