...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'retry-deploy') {
      ...
      if (task.state !== 'deploy_failed') {
        send(res, 409, { error: `Task must have a failed deployment before retry. Current state: ${task.state}.`, task });
        return;
      }
      const plan = deploymentPlan(task);
      const deployment = await retryDeployment(task, plan, 'main');
      ...
      task.deployment = {
        ...
        state: 'deploying',
        dispatchedAt,
        retryCount: Number(task.deployment?.retryCount || 0) + 1,
        run: null,
        conclusion: null
      };
      task.updatedAt = dispatchedAt;
      task.events = Array.isArray(task.events) ? task.events : [];
      task.events.push({ type: 'deployment.retry.dispatched', at: dispatchedAt, workflow: deployment.workflow, environment: deployment.environment, ref: deployment.ref, retryCount: task.deployment.retryCount, source: 'zvelclaw-web' });
      const saved = await saveTask(owner, repo, task, result.sha, `task: retry deployment ${task.id}`);
    ...
    if (action === 'execute') {
      ...
      task.gate = gate; task.state = gate.passed ? 'gated' : 'failed';
      ...
      send(res, 200, { task, review, gate, commit: saved?.commit?.sha || null }); return;
    }
    ...
    if (action === 'create-pr') {
      ...
      if (!result) { send(res, 404, { error: 'Task not found.' }); return; }
      const task = result.task;
      if (task.state !== 'gated' && task.state !== 'pr_created') { send(res, 409, { error: `Task must be gated before PR creation. Current state: ${task.state}.`, task }); return; }
      if (task.github?.prNumber) { send(res, 409, { error: `PR #${task.github.prNumber} already exists.`, task }); return; }
      const githubResult = await createGitHubPR(task, githubPlan(task));
      if (githubResult.skipped) { send(res, 503, { error: githubResult.reason, task }); return; }
      task.github = githubResult; task.state = 'pr_created'; task.updatedAt = new Date().toISOString(); task.events = Array.isArray(task.events) ? task.events : [];
    ...
    if (action === 'ready-pr') {
      const result = await getTaskFile(owner, repo, input.id);
      if (!result) { send(res, 404, { error: 'Task not found.' }); return; }
      const task = result.task; const prNumber = Number(task.github?.prNumber);
      ...
      task.github = { ...(task.github || {}), draft: false }; task.updatedAt = new Date().toISOString(); task.events = Array.isArray(task.events) ? task.events : [];
      ...
      send(res, 200, { task, ready, commit: saved?.commit?.sha || null }); return;
    }
    ...
    if (action === 'check-pr') {
      ...
      task.github = { ...(task.github || {}), gate }; task.updatedAt = new Date().toISOString(); task.events = Array.isArray(task.events) ? task.events : [];
    ...
    if (action === 'merge-pr') {
      ...
      task.github = { ...(task.github || {}), gate, merged: true, mergeMethod: method, mergeSha: merged.sha }; task.state = 'merged'; task.updatedAt = new Date().toISOString(); task.events = Array.isArray(task.events) ? task.events : [];
    ...
    if (action === 'deploy-task') {
      ...
      if (!result) { send(res, 404, { error: 'Task not found.' }); return; }
      const task = result.task;
      if (task.state !== 'merged') { send(res, 409, { error: `Task must be merged before deployment. Current state: ${task.state}.`, task }); return; }
      const deployment = await triggerDeployment(task, deploymentPlan(task), 'main');
      if (deployment.skipped) { send(res, 503, { error: deployment.reason, task }); return; }
      const dispatchedAt = new Date().toISOString(); task.state = 'deploying';
      task.deployment = { ...(task.deployment || {}), provider: deployment.provider, workflow: deployment.workflow, environment: deployment.environment, repository: deployment.repository, ref: deployment.ref, status: 'dispatched', state: 'deploying', dispatchedAt, retryCount: 0, run: null };
      task.updatedAt = dispatchedAt; task.events = Array.isArray(task.events) ? task.events : [];
      ...
      send(res, 200, { task, deployment: task.deployment, commit: saved?.commit?.sha || null }); return;
    }

    const description = String(input.description || '').trim();
    const executionMode = input.executionMode === 'review' ? 'review' : 'local';
    if (description.length < 8) { send(res, 400, { error: 'Task description must contain at least 8 characters.' }); return; }
    const now = new Date().toISOString();
    const id = `${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
    const task = { id, description, project: `${owner}/${repo}`, executionMode, state: 'created', createdAt: now, events: [{ type: 'task.created', at: now, source: 'zvelclaw-web' }] };
    const saved = await saveTask(owner, repo, task, undefined, `task: create ${task.id}`);
    send(res, 201, { task, commit: saved?.commit?.sha || null });
  } catch (error) {
