...
async function getTaskFile(owner, repo, id) {
  ...
  try {
    const file = await github(
      `/repos/${owner}/${repo}/contents/.zvelclaw/tasks/${encodeURIComponent(safe)}.json`
    );
...
async function saveTask(owner, repo, task, sha, message) {
  const path =
    `.zvelclaw/tasks/${safeId(task.id)}.json`;
...
async function listTasks(owner, repo) {
  ...
  try {
    entries = await github(
      `/repos/${owner}/${repo}/contents/.zvelclaw/tasks`
    );
  } catch (error) {
    if (/not found/i.test(error.message)) {
      return [];
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      if (id) {
        const result = await getTaskFile(
          owner,
          repo,
          id
        );
     ...
     * EXECUTE
     *
     * created -> executing -> review -> gated
     */
    if (action === 'execute') {
      const result = await getTaskFile(
        owner,
      ...
      if (
        ...
        ['executing', 'review'].includes(task.state)
      ) {
        send(res, 409, {
          error:
    ...
    if (action === 'create-pr') {
      ...
      if (
        ...
        send(res, 409, {
          error:
            `Task must be gated before PR creation. Current state: ${task.state}.`,
          task
        });
      ...
      if (task.github?.prNumber) {
        send(res, 409, {
          error:
            `PR #${task.github.prNumber} already exists.`,
          task
        });
    ...
    if (action === 'merge-pr') {
      ...
      if (!gate.passed) {
        send(res, 409, {
          error:
            `Merge blocked by Gate: ${gate.reason}`,
          task,
          gate
        });
    ...
    if (action === 'deploy-task') {
      ...
      if (task.state !== 'merged') {
        send(res, 409, {
          error:
            `Task must be merged before deployment. Current state: ${task.state}.`,
          task
        });
    ...
    if (description.length < 8) {
      send(res, 400, {
        error:
          'Task description must contain at least 8 characters.'
      });
